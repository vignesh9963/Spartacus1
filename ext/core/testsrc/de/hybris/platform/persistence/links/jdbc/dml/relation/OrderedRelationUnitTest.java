...
public class OrderedRelationUnitTest
{
	...
	public void shouldGenerateRemoveOperation()
	{
		...
		assertThat(operations).containsOnly(r.remove(2, 'B'));
	}
	...
	public void shouldGenerateRemoveOperationsForAllChildren()
	{
		...
		assertThat(operations).containsOnly(r.remove(2, 'B'), r.remove(3, 'C'), r.remove(1, 'A'));
	}
	...
	public void shouldInsertChildrenAtTheBeginningAndShiftExistingLinks()
	{
		...
		assertThat(operations).containsOnly(
				r.add('D', 0),
				r.add('E', 1),
				r.shift(1, 2, 'A'),
				r.shift(2, 3, 'B'),
	...
	public void shouldRemoveAllExistingLinksWhenSettingToEmptyCollection()
	{
		...
		assertThat(operations).isNotNull().hasSize(3);
		assertThat(operations).containsOnly(r.remove(1, 'A'), r.remove(2, 'B'), r.remove(3, 'C'));
	}
	...
	public void shouldReusePartOfExistingLinks()
	{
		...
		assertThat(operations).isNotNull().hasSize(2);
		assertThat(operations).containsOnly(r.remove(1, 'A'), r.remove(3, 'C'));
	}
	...
	public void shouldShiftWhenPositionOfExistingLinkMustBeChanged()
	{
		...
		assertThat(operations).isNotNull().hasSize(2);
		assertThat(operations).containsOnly(r.add('F', 8), r.shift(2, 9, 'B'));
	}
	...
	public void shouldBeSmart()
	{
		...
		assertThat(operations).isNotNull().hasSize(2);
		assertThat(operations).containsOnly(r.shift(1, 11, 'A'), r.shift(3, 12, 'B'));
	}
	...
	public void shouldNotTouchShiftedChildWhenInsertingNewLink()
	{
		...
		assertThat(operations).isNotNull().isNotEmpty();
		assertThat(operations).containsOnly(r.add('B', 1), r.touchedParent(), r.touchedChild('B'), r.shift(2, 2, 'C'));
	}
	...
	private static class TestableOrderedRelation extends OrderedRelation
	{
		...
		public RelationModification shift(final long id, final int toPosition, final char ch)
		{
			return new Shift(id, 0, toPosition, (long) ch);
		}
		...
		private RelationModification remove(final long id, final char ch)
		{
			return new Remove(id, 0, (long) ch);
		}
	}
