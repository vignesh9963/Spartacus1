...
public class AuditRecordExtractorTest
{
	...
	public void testExtractDataWithPayloadSearchRuleWithEqualValue() throws SQLException
	{
		...
		Mockito.lenient().when(jsonAuditRecordA.getAttributeAfterOperation(FIELD_A)).thenReturn(FIELD_A_NEW_VALUE);
	...
	public void testExtractDataWithPayloadSearchRuleWithNoneEqualValue() throws SQLException
	{
		...
		Mockito.lenient().when(auditRecordRowMapper.mapRow(eq(resultSet), anyInt())).thenReturn(jsonAuditRecordA);

		when(resultSet.next()).thenReturn(true, true, false);

		Mockito.lenient().when(jsonAuditRecordA.getAttributeBeforeOperation(FIELD_A)).thenReturn(FIELD_A_OTHER_VALUE);
		Mockito.lenient().when(jsonAuditRecordA.getAttributeAfterOperation(FIELD_A)).thenReturn(FIELD_A_OTHER_NEW_VALUE);
