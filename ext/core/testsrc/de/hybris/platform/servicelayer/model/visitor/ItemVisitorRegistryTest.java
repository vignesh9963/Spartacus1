...
public class ItemVisitorRegistryTest
{
	...
	public void setUp()
	{
		...
		Mockito.lenient().when(typeService.getComposedTypeForCode("Item")).thenReturn(itemComposedType);

		Mockito.when(productComposedType.getAllSuperTypes()).thenReturn(Lists.newArrayList(itemComposedType));
		Mockito.when(variantComposedType.getAllSuperTypes())
		       .thenReturn(Lists.newArrayList(productComposedType, itemComposedType));
