...
public class DefaultGenericDaoIntegrationTest extends ServicelayerBaseTest
{
	...
	private static class IsValidPolyglotQuery implements ArgumentMatcher<FlexibleSearchQuery>
	{
		@Override
		public boolean matches(final FlexibleSearchQuery fsq)
		{
			return PolyglotPersistenceFlexibleSearchSupport
					.tryToConvertToPolyglotCriteria(
							QueryOptions.newBuilder().withQuery(fsq.getQuery()).withValues(fsq.getQueryParameters()).build())
					.isPresent();
		}
	}
