...
public class DefaultCorsConfigurationSourceTest extends ServicelayerBaseTest
{
	...
	public void testDefaultConfiguration()
	{
		...
		assertThat(corsConfiguration.checkHttpMethod(HttpMethod.DELETE)).isNotEmpty();
		assertThat(corsConfiguration.checkHttpMethod(HttpMethod.GET)).isNull();
		assertThat(corsConfiguration.getAllowCredentials()).matches(value -> !Boolean.TRUE.equals(value));
	}
	...
	{
		// given
		setParameter(CORSFILTER + DOT + WEBAPP + DOT + CorsConstants.Key.allowedOriginPatterns, "http://*.hybris.com");

		// when
		final CorsConfiguration corsConfiguration = defaultCorsConfigurationSource.getCorsConfiguration(createRequest(WEBAPP));

		// then
		assertThat(corsConfiguration.checkOrigin("http://dummy.hybris.com")).isNotEmpty();
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM)).isNull();
	}
	...
	{
		// given
		setParameter(CORSFILTER + DOT + WEBAPP + DOT + CorsConstants.Key.allowedOriginPatterns, HTTP_HYBRIS_COM);

		// when
		final CorsConfiguration corsConfiguration = defaultCorsConfigurationSource.getCorsConfiguration(createRequest(WEBAPP));

		// then
		assertThat(corsConfiguration.checkOrigin("http://dummy.hybris.com")).isNull();
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM)).isNotEmpty();
	}
	...
	{
		// given
		setParameter(CORSFILTER + DOT + WEBAPP + DOT + CorsConstants.Key.allowedOriginPatterns, HTTP_HYBRIS_COM + ":[80\\,8080]");

		// when
		final CorsConfiguration corsConfiguration = defaultCorsConfigurationSource.getCorsConfiguration(createRequest(WEBAPP));

		// then
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM + ":80")).isNotEmpty();
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM + ":8080")).isNotEmpty();
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM + ":8081")).isNull();
	}
	...
	public void testAllowedOriginPatternsWithAllPorts()
	{
		...
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM + ":80")).isNotEmpty();
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM + ":8080")).isNotEmpty();
		assertThat(corsConfiguration.checkOrigin(HTTP_HYBRIS_COM + ":8081")).isNotEmpty();
	}
	@Test
	public void testCachePerformance()
	{
		final CorsPropertiesLoader propertiesLoader = defaultCorsConfigurationSource.getPropertiesLoader();
