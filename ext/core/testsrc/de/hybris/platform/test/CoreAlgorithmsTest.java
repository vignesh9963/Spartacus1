...
public class CoreAlgorithmsTest
{
	...
	public void testConvertAndRound()
	{
		...
		assertEquals("28.635",
				Double.toString(
						CoreAlgorithms.convert(srcRatio.doubleValue(), tgtRatio.doubleValue(), price.doubleValue())));
		assertEquals("28.64", Double.toString(CoreAlgorithms
				.round(CoreAlgorithms.convert(srcRatio.doubleValue(),
						tgtRatio.doubleValue(), price.doubleValue()),
						2)));
	}
	...
	{
		//given
		final double sourceFactor = 0.0;
		final double targetFactor = 2.32;
		final double value = 1.23;

		//when, then
		assertThatThrownBy(() -> CoreAlgorithms.convert(sourceFactor, targetFactor, value)).isInstanceOf(
				IllegalArgumentException.class).hasMessageContaining("currency/unit conversion factor must never be 0");
	}
	...
	{
		//given
		final double sourceFactor = 2.32;
		final double targetFactor = 0.0;
		final double value = 1.23;
