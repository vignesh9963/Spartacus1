...
public class DefaultAuditViewServiceCustomResolverTest extends ServicelayerBaseTest
{
	...
	public void shouldDelegateToCustomReferenceResolver()
	{
		...
		final TypeAuditReportConfig config = TypeAuditReportConfig.builder() //
		                                                          .withConfig(auditTestHelper.loadConfigFromFile(
				                                                          "audit.test/custom-resolvers-reference.xml",
				                                                          "UserReport")) //
		                                                          .withFullReport() //
	...
	public void shouldOverwriteExpressionInResolvesByTag()
	{
		...
		final TypeAuditReportConfig config = TypeAuditReportConfig.builder() //
		                                                          .withConfig(auditTestHelper.loadConfigFromFile(
				                                                          "audit.test/custom-resolvers-reference-overwrite-expression.xml",
				                                                          "UserReport")) //
		                                                          .withFullReport() //
		                                                          .withRootTypePk(user.getPk()) //
		                                                          .build();
		...
		verify(virtualReferenceDummyResolver, atLeast(1)).resolve(any(),
				argThat(new ExpressionArgumentMatcher("virtualExpression")),
				anyCollectionOf(AuditRecord.class));
	}


	private static class ExpressionArgumentMatcher
			implements ArgumentMatcher<Map<AbstractTypedAttribute, AuditTypeContext<AuditRecord>>>
	{
		...
		public boolean matches(
				final Map<AbstractTypedAttribute, AuditTypeContext<AuditRecord>> abstractTypedAttributeAuditTypeContextMap)
		{
			return abstractTypedAttributeAuditTypeContextMap.keySet().stream().map(AbstractTypedAttribute::getResolvesBy)
