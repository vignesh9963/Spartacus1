...
public class DefaultClassificationServiceTest
{
	...
	{
		// given
		Mockito.lenient().when(classificationClassesResolverStrategy.resolve(product)).thenReturn(Collections.EMPTY_SET);
		Mockito.lenient()
		       .when(classificationClassesResolverStrategy.getAllClassAttributeAssignments(Collections.EMPTY_SET))
		       .thenReturn(
				       Collections.EMPTY_LIST);
