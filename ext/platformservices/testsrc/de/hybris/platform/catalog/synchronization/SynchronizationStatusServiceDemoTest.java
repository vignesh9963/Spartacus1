...
public class SynchronizationStatusServiceDemoTest extends BaseSynchronizationStatusServiceTest
{
	...
	public void setUp()
	{
		...
		sourceCatalogVersion = createCatalogVersion(catalog,
				String.format("%s%s", "test_source_version",
						RandomStringUtils.randomAlphanumeric(3)));
		targetCatalogVersion = createCatalogVersion(catalog,
				String.format("%s%s", "test_target_version",
						RandomStringUtils.randomAlphanumeric(3)));
	...
	{
		Mockito.lenient().when(visitors.keySet()).thenReturn(visitorRegistry.keySet());
		for (final String theKey : visitorRegistry.keySet())
		{
			Mockito.lenient().when(visitors.get(theKey)).thenReturn(visitorRegistry.get(theKey));
