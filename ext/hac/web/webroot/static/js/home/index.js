...
//		charts[$("#sessionsChartContainer").attr("chartId")] = new Chart($("#sessionsChartContainer"), $("#sessionsChartLabels"), 0, "", 3, true);
	...
	function readCookieValues(){
		...
		var settings = JSON.parse(Cookies.get(cookieName));
	...
	function createCookie(content){
		Cookies.set(cookieName, content, { expires: 365 });
	}
	
	function buildHistory() {
        const historyArr = hac.history.getSortedTruncatedHistory(5);
    ...
    function createListItems(item) {
        for (const pos2 in item) {
            if (item.hasOwnProperty(pos2)) {
                const data = item[pos2];
                $('#toplinks').append($("<li>").append($("<a>", {
                    class: "quiet",
                    href: hac.contextPath + pos2
                }).text(data.t)));
            }
        }
    }
