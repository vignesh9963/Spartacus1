...
public class ImpExImportTest extends AbstractImpExTest
{
	...
	{
		final de.hybris.platform.jalo.security.AccessManager accessManager = de.hybris.platform.jalo.security.AccessManager.getInstance();
	...
	public void testMergeAddsNewItemWhenItDoesntYetExist() throws ConsistencyCheckException, ImpExException
	{
		...
		final String data = "UPDATE Language ; isoCode[unique=true]; fallbackLanguages(isoCode) \n" // h1
				+ "; l1 ; (+)f1 \n" //
				+ "; l1 ; (+?)f2 \n" //
				;
		...
		Language updatedLang = (Language) reader.readLine();
		assertEquals(language1, updatedLang);
		assertCollection(Collections.singletonList(fallback1), updatedLang.getFallbackLanguages());

		updatedLang = (Language) reader.readLine();
		assertEquals(language1, updatedLang);
		assertCollection(Arrays.asList(fallback1, fallback2), updatedLang.getFallbackLanguages());
	}

	@Test
	public void testMergeDoesntAddTheSameItemTwice() throws ConsistencyCheckException, ImpExException
	{
		// create some languages
		final Language language1 = C2LManager.getInstance().createLanguage("l1");
	...
	public void testMergeOnColumnLevelDoesntAddTheSameItemTwice() throws ConsistencyCheckException, ImpExException
	{
		...
		// test replacing, adding and removing of collection elements
		final String data = "UPDATE Language ; isoCode[unique=true]; fallbackLanguages(isoCode)[mode=merge] \n" // h1
				+ "; l1 ; f1 \n" //
				+ "; l1 ; f2 \n" //
				+ "; l1 ; f2 \n" //
				;
