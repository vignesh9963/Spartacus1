...
public class TaskServiceTest extends AbstractTaskTest
{
	...
	private final TestScriptLogicHolder scriptLogicHolder = new TestScriptLogicHolder();
	...
	public void testTX() throws InterruptedException
	{
		try
		{
			TestUtils.disableFileAnalyzer("Exception in task", 100);
			...
			try
			{
				...
				assertFalse("processed task item has not been removed after " + timeFactor + " seconds",
						item.isAlive()); // *now* it must be removed or this is a error !!!
			}
	...
	@Test
	public void shouldLogErrorIfNotCaughtByRunStrategy()
	{
		final String errorId = UUID.randomUUID().toString();

		final ScriptModel scriptModel = scriptLogicHolder.scriptWithLogic(modelService, () -> {
			throw new IllegalAccessError(errorId);
		});

		final ScriptingTaskModel task = modelService.create(ScriptingTaskModel.class);

		task.setScriptURI("model://" + scriptModel.getCode());
		modelService.saveAll();

		final HybrisLogListener newListener = mock(HybrisLogListener.class);
		when(newListener.isEnabledFor(any())).thenReturn(true);
		HybrisLogger.addListener(newListener);

		try
		{
			taskService.scheduleTask(task);

			verify(newListener, timeout(TimeUnit.SECONDS.toMillis(30)).atLeastOnce()).log(argThat(
					hasProperty("throwable",
							allOf(isA(IllegalAccessError.class), hasProperty("message", containsString(errorId))))));
		}
		finally
		{
			modelService.remove(task);
			HybrisLogger.removeListener(newListener);
		}
	}
