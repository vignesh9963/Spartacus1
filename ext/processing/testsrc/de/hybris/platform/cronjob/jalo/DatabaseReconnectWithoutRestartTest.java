...
public class DatabaseReconnectWithoutRestartTest extends HybrisJUnit4Test
{
	...
	public static final String TENANT_CONNECTION_CHECK_ENABLED = "tenant.connection.check.enabled";

	private Tenant tenantBefore;
	private Tenant foo;
	private String beforeTenantConnectionCheckEnabled;

	@Before
	public void prepare()
	{
		tenantBefore = Registry.getCurrentTenantNoFallback();
		this.foo = Registry.getTenantByID("foo");
		this.beforeTenantConnectionCheckEnabled = foo.getConfig().getParameter(TENANT_CONNECTION_CHECK_ENABLED);
		this.foo.getConfig().setParameter(TENANT_CONNECTION_CHECK_ENABLED, "true");
		Registry.setCurrentTenant(foo); 
	}

	@After
	public void unprepare()
	{
		this.foo.getConfig().setParameter(TENANT_CONNECTION_CHECK_ENABLED, beforeTenantConnectionCheckEnabled);
