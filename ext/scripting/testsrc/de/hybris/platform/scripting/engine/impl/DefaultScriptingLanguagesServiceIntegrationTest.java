...
public class DefaultScriptingLanguagesServiceIntegrationTest extends ServicelayerBaseTest
{
	...
	private void prepareScriptModels() throws FileNotFoundException
	{
		...
		prepareScriptModel("bazBarDisabled", "spring.getBean \"modelService\"", true);
	}

	private ScriptModel prepareScriptModel(final String code, final String content, final boolean disabled)
	{
	...
	{
		return prepareScriptModel(code, content, false);
	}
	...
	@Test(expected = DisabledScriptException.class)
	public void shouldThrowExceptionWhenExecutingDisabledSimpleGroovyScriptBackedByModel()
	{
		// given
		final String scriptURI = "model://bazBarDisabled";
		final Map<String, Object> ctx = new HashMap<>();
		ctx.put("spring", Registry.getApplicationContext());
